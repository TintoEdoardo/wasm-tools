...
use wasm_encoder::{BlockType, CodeSection, DataCountSection, DataSection, ElementSection, Elements, EntityType, ExportKind, ExportSection, Function, FunctionSection, GlobalSection, GlobalType, ImportSection, Instruction, MemArg, MemorySection, MemoryType, Module, ModuleSection, NestedComponentSection, TypeSection, ValType};
...
impl<'a> WasmMigrate<'a> {
    ...
    fn encode_module(module_data: &ModuleData<'a>, func_index: usize, inject: bool, distributed: bool) -> wasm_encoder::Module
    {
        ...
        /*
        let mut memories = MemorySection::new();
        if let Some(memory_section) = module_data.memory_section.clone() {
        ...
        match &module_data.data_count_section {
            Some(count) => {
                module.section(&crate::DataCountSection { count: *count });
            }
            None => {
                // Nothing to be done.
            }
        ...
        if inject {
            ...
            /* for i in 0..module_data.function_bodies.len() {
                let func_body = module_data.function_bodies[i].clone();
                let locals = module_data.locals[i].clone();
...
mod tests {
    ...
    fn add_checkpoint_at() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut file = File::open("./tests/guest.wasm").unwrap();
        ...
        migration_injector.core_module_index = 3;
        migration_injector.func_index = 5;
